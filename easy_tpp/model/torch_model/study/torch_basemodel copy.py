...
class TorchBaseModel(nn.Module):
    ...
    def generate_model_from_config(model_config):
        ...
        # classe modificada para retornar também netos, já que o RoTHP herda do THP e não de TorchBaseModel
        ...
        # Check for Residual TPP flag
        # If 'use_residual' is set in model_config (or model_specs), we wrap the target model.
        # We access the dictionary directly or the attribute if it exists.
        use_residual = getattr(model_config, 'use_residual', False)
        # model_specs is a dict, sometimes keys are there
        if not use_residual and 'use_residual' in model_config.model_specs:
             use_residual = model_config.model_specs['use_residual']

        if use_residual:
            from easy_tpp.model.torch_model.torch_residual_tpp import ResidualTPP
            return ResidualTPP(model_config)
