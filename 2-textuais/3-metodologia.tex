...
A escolha dessa normalização foi feita após avaliarmos duas alternativas. A primeira, uma normalização MinMax aplicada aos intervalos entre os eventos que transforma cada $\Delta t_i = t_i - t_{i-1}$ para o intervalo $[0, 1]$ dividindo pelo intervalo máximo da sequência, mostrou-se inadequada para o protocolo \textit{train-short, test-long}. Nos experimentos sintéticos com o protocolo de extrapolação, essa normalização favoreceu sistematicamente o NHP, que não depende de embeddings posicionais explícitos, enquanto os modelos com mecanismos de atenção temporal (THP, RoTHP, HoTHP) foram prejudicados ou apresentaram instabilidade, especialmente nos fatores de extrapolação elevados.

A normalização por média não sofre desse problema porque o divisor $\bar{g}$ é a média dos intervalos, que se mantém estável ao adicionar mais eventos de um mesmo processo. Um eventual intervalo atípico também é diluído pela média, ao passo que no MinMax ele alteraria o máximo e comprimiria todos os demais.
