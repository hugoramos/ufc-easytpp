...
Como vimos na Seção \ref{sec:processos-de-hawkes-utilizando-transformers}, o RoTHP representa a posição de cada evento por uma rotação trigonométrica aplicada aos vetores de query $\mathbf{q}$ e key $\mathbf{k}$. O score de atenção entre os eventos $i$ e $j$ depende apenas de $\Delta t = t_i - t_j$, o que é garantido pelas propriedades algébricas da matrize de rotação trigonométrica, que é ortogonal.
...
A escolha dessa normalização foi feita após avaliarmos duas alternativas. A primeira, uma normalização MinMax que transforma os instantes para o intervalo $[0, 1]$, dividindo cada timestamp pelo instante máximo da sequência, mostrou-se inadequada para o protocolo \textit{train-short, test-long}. Como o instante máximo corresponde à soma de todos os intervalos da sequência, esse divisor cresce com o número de eventos. Assim, quanto mais longa a sequência de teste, menores ficam os intervalos que o modelo observa. Um mesmo intervalo real entre dois eventos passa a ser representado por um valor menor no teste do que no treino, simplesmente porque a sequência é mais longa. Além disso, como o kernel hiperbólico calcula exponenciais esses intervalos ficam muito pequenos. Na prática, o mecanismo de atenção passa a atribuir peso semelhante a todos os eventos da sequência, eliminando o viés de recência e tornando os modelos indistinguíveis entre si.

A normalização por média não sofre desse problema porque o divisor $\bar{g}$ é a média dos intervalos, não a soma. Ao adicionar mais eventos de um mesmo processo, a soma dos intervalos cresce, mas a média se mantém estável. Um eventual intervalo atípico também é diluído pela média, ao passo que no MinMax ele alteraria a soma e comprimiria todos os demais. Com isso, um mesmo intervalo real é representado por valores semelhantes no treino e no teste e o modelo vê números na mesma faixa que aprendeu.
...
Para cada cenário, geramos 500 sequências de treinamento, 150 de validação e 200 de teste. Todas as sequências são pré-processadas com a normalização temporal da Equação~\ref{eq:normalizacao-temporal} antes de alimentar os modelos.
...
Repetimos todos os experimentos com $N = 5$ seeds e reportamos média $\pm$ desvio padrão. Para verificar significância estatística, usamos o teste de Wilcoxon de postos sinalizados (\textit{Wilcoxon signed-rank test}), um teste não paramétrico pareado que não assume normalidade e é adequado para $N = 5$.
