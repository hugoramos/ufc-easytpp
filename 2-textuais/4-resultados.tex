...
% TODO: preencher com os resultados do notebook de validação sintética
\begin{tabular}{llcccc}
\toprule
Cenário & Modelo & $1\times$ & $2\times$ & $5\times$ & $10\times$ \\
\midrule
\multirow{2}{*}{Decaimento lento} & RoTHP & --- & --- & --- & --- \\
                                   & HoTHP & --- & --- & --- & --- \\
\midrule
\multirow{2}{*}{Decaimento rápido} & RoTHP & --- & --- & --- & --- \\
                                    & HoTHP & --- & --- & --- & --- \\
\bottomrule
\end{tabular}
\end{table}

% TODO: adicionar Figura com gráfico NLL × fator de extrapolação
...
\begin{enumerate}
    \item \textbf{Amazon: padrão auto-inibitório.} A intensidade exibe um padrão de ``dente de serra'': cresce entre eventos e cai abruptamente quando um evento ocorre. Esse comportamento indica um processo auto-inibitório, onde a ocorrência de um evento reduz temporariamente a probabilidade de novos eventos. É o oposto do processo de Hawkes clássico, onde eventos excitam a intensidade. O parâmetro $\alpha_k$ na Equação~\ref{eq:intensidade-hothp} é positivo neste caso, fazendo $\lambda^*(t)$ crescer com o tempo desde o último evento.

    \item \textbf{Stackoverflow: intensidade quase constante.} Os três modelos aprendem uma intensidade praticamente plana, sem saltos nos eventos. Esse padrão é característico de um processo de Poisson homogêneo, onde eventos ocorrem independentemente. Se não há estrutura temporal, nenhum viés posicional, seja oscilatório (RoTHP) ou de decaimento (HoTHP), consegue explorar informação adicional.

    \item \textbf{Retweet: colapso numérico.} Ambos os modelos mostram spikes apenas no burst inicial (onde os eventos estão concentrados) e depois $\lambda^*(t) \approx 0$ pelo restante da sequência. Os intervalos entre eventos são tão grandes (dezenas de milhares de unidades) que qualquer decaimento, exponencial ou não, satura para zero. Os modelos ficam efetivamente cegos para a maior parte da sequência.
\end{enumerate}

A observação do Amazon é particularmente relevante à luz da Seção~\ref{sec:onde-atua-vies}: o HoTHP vence nesse dataset apesar de o processo ser auto-inibitório, não auto-excitante. Isso confirma que o viés de recência atua na atenção (``quais eventos passados são mais relevantes para construir a representação interna'') e não na intensidade (``a intensidade sobe ou desce após um evento''). No Amazon, o evento mais recente é o mais informativo para prever o próximo, e o decaimento exponencial na atenção captura isso corretamente.
...
A normalização resolve o colapso numérico de forma inequívoca. Com timestamps brutos, a NLL em $1\times$ é de aproximadamente $12{,}5$ para ambos os modelos (2 de 3 sementes colapsam para um ponto degenerado com NLL $= 15{,}94$) e a NLL em $5\times$ explode para a ordem de $10^5$. Com timestamps normalizados, a NLL em $1\times$ cai para valores negativos (indicando boa modelagem) e o treino converge em todas as sementes.
