...
\institute[UFC]{Universidade Federal do Ceará \\[0.4em]
...
\begin{frame}{The Poisson distribution}
  ...
  {\small
  \begin{itemize}
    \item $\lambda$ is both the \textbf{mean} and the \textbf{variance} of the
          count.
    \item Approximates a binomial: $n$ \textbf{trials}, each succeeding with
          \textbf{small} probability $p$; expected count $\lambda = np$ (e.g.,
          $n$ characters on a page, each misprinted with probability $p$).
  \end{itemize}}
\end{frame}
