...
\begin{table}[h!]
\centering
\caption{NLL (média $\pm$ desvio padrão, 10 sementes) nos dados sintéticos. Valores menores indicam melhor desempenho. O melhor resultado para cada configuração está em negrito.}
...
\begin{figure}[h!]
    ...
    \Caption{\label{fig:extrapolacao-sintetica} NLL (média $\pm$ desvio padrão, 10 sementes) em função do fator de extrapolação $\alpha$ nos cenários sintéticos. O HoTHP mantém a NLL mais estável à medida que $\alpha$ cresce, enquanto o THP degrada drasticamente e o RoTHP e NHP apresentam degradação intermediária.}
    \UFCfig{}{
...
Com a inclusão do NHP e do THP como baselines, quatro comportamentos distintos emergem na extrapolação. O THP é o modelo que mais degrada: sua NLL salta para $\approx 3{,}9$ já em $2\times$ e permanece entre $\approx 3{,}9$ e $\approx 4{,}2$ nos fatores maiores, nos dois cenários, com desvio padrão muito alto (até $\pm 2{,}12$) causado por sementes que divergem completamente. Isso confirma que o encoding temporal absoluto do THP produz representações fora da distribuição para posições novas. O NHP, apesar de não usar atenção, é o baseline mais competitivo na extrapolação, provavelmente porque sua arquitetura recorrente acumula informação de forma incremental, sem depender de posições absolutas; ainda assim, fica atrás do HoTHP em todos os fatores de extrapolação nos dois cenários.

O HoTHP obtém a melhor NLL em todos os fatores de extrapolação ($2\times$, $5\times$ e $10\times$) nos dois cenários. O custo dessa vantagem aparece em distribuição: em $1\times$, o HoTHP é o modelo com pior NLL (por exemplo, $1{,}545$ contra $1{,}470$ do NHP no decaimento lento), uma diferença pequena que se inverte assim que a sequência de teste ultrapassa o comprimento de treino. No decaimento rápido, a NLL do HoTHP cresce apenas $0{,}085$ entre $1\times$ e $10\times$ ($1{,}689 \to 1{,}774$), permanecendo praticamente plana, enquanto o segundo melhor modelo (NHP) cresce $0{,}490$ ($1{,}663 \to 2{,}153$). Essa estabilidade é esperada: como o processo gerador possui kernel exponencial, o viés de recência do HoTHP está perfeitamente alinhado com a estrutura real dos dados. Para distâncias temporais não vistas no treino, o kernel hiperbólico continua a decair monotonicamente, enquanto o kernel trigonométrico do RoTHP oscila em fases não aprendidas e o encoding absoluto do THP diverge.

A vantagem do HoTHP sobre os três baselines é estatisticamente significativa (teste de Wilcoxon de postos sinalizados, pareado por semente e unilateral, $p < 0{,}05$) em todos os fatores de extrapolação dos dois cenários, sem exceção. O caso menos folgado é o decaimento lento em $10\times$ frente ao NHP ($p = 0{,}042$); em todas as demais comparações o valor-$p$ fica em $0{,}025$ ou abaixo, atingindo o piso de $0{,}001$ (igual a $1/2^{10}$, o menor valor possível para o teste unilateral com 10 sementes) na maioria delas.

No decaimento lento, onde a influência de eventos distantes persiste por mais tempo, o HoTHP ainda obtém a melhor NLL média em todos os fatores, mas sua degradação é muito maior do que no decaimento rápido: a NLL cresce $0{,}819$ entre $1\times$ e $10\times$, contra apenas $0{,}085$ no decaimento rápido. Em $10\times$, a média do HoTHP ($2{,}364$) é inferior à do NHP ($2{,}817$) e a diferença permanece estatisticamente significativa, ainda que pela margem mais estreita de todo o experimento ($p = 0{,}042$), reflexo da alta variância entre sementes nesse regime ($\pm 0{,}625$). Isso sugere que, quando a memória do processo é longa, o decaimento exponencial imposto pelo kernel hiperbólico pode descartar prematuramente eventos ainda relevantes, reduzindo a margem de vantagem.

A Figura~\ref{fig:extrapolacao-rmse} mostra o RMSE da predição do próximo evento. No decaimento lento, todos os modelos degradam de forma similar (RMSE $\approx 2{,}7$--$2{,}8$ em $10\times$), com o HoTHP apresentando o menor RMSE ($2{,}714$). No decaimento rápido, os quatro modelos mantêm RMSE estável e próximo (entre $\approx 1{,}40$ e $1{,}55$ em $10\times$): o RoTHP obtém o menor valor ($1{,}407$) e o NHP o maior ($1{,}549$), este último com variância um pouco mais alta por conta de uma semente atípica. Diferentemente da NLL, o RMSE não separa nitidamente os modelos. Isso indica que a vantagem do HoTHP se manifesta sobretudo na verossimilhança, ou seja, na modelagem da distribuição temporal completa, e não na predição pontual do próximo intervalo, em que os modelos de atenção ficam praticamente empatados.

\begin{figure}[h!]
    \centering
    \captionsetup{width=14cm}
    \Caption{\label{fig:extrapolacao-rmse} RMSE da predição do próximo evento (média $\pm$ desvio padrão, 10 sementes) em função do fator de extrapolação $\alpha$. Diferentemente da NLL, o RMSE não separa nitidamente os modelos: no decaimento lento todos degradam de forma similar e no decaimento rápido permanecem próximos entre si.}
    \UFCfig{}{
