...
\begin{figure}[h!]
    ...
    \caption[NLL versus extrapolation factor on synthetic data]{\label{fig:extrapolacao-sintetica} NLL (mean $\pm$ standard deviation, 10 seeds) as a function of the extrapolation factor $\alpha$ in the synthetic scenarios. The HoTHP keeps the NLL more stable as $\alpha$ grows, while the THP degrades drastically and the RoTHP and NHP show intermediate degradation.}
    \UFCfig{}{
...
Table~\ref{tab:resultados-reais} presents the NLL and Table~\ref{tab:acc-reais} the type-prediction accuracy.
...
It is worth being careful about what the slow decay scenario tells us. Both synthetic scenarios are decaying processes, one fast and one slow, and the HoTHP wins both at every factor beyond the training length, precisely because the decay rate of its kernel is learnable. The parameter $\theta'$ can be pushed close to its lower bound so that the slowest frequency pair decays very gently, so the model can keep attending to older events when the data asks for it. A slower memory is therefore not, in itself, a problem for the HoTHP.

What the slow decay does show is that extrapolation gets harder for every model as the memory lengthens. The absolute NLL of the HoTHP grows by $0{,}819$ between $1\times$ and $10\times$ in the slow decay, against only $0{,}085$ in the fast decay, and the variance between seeds at $10\times$ is much higher ($\pm 0{,}625$), which produces the least comfortable significance of the whole experiment ($p = 0{,}042$). The HoTHP still keeps the best mean NLL, with a lead over the NHP and the RoTHP that is in fact slightly wider than in the fast decay, so it does not lose the comparison. But its curve is less flat, which suggests that a single global $\theta'$, calibrated on training sequences of only 50 events, is harder to set well when the relevant temporal range extends far beyond what was seen in training.

The clearest limit of the recency bias is a different regime, one that the synthetic experiments do not cover. Both scenarios decay monotonically, which is exactly the shape the hyperbolic kernel is built for. When the dependence is not monotonic, for example when distant events are as informative as recent ones, or when the pattern is periodic, no value of $\theta'$ can help, because the kernel can only decrease with the distance. This is reinforced by the evaluation protocol itself. The \textit{train-short, test-long} setup rewards, almost by construction, models with a recency bias: short-range structure is invariant to the length of the sequence, so a model that relies on recent events extrapolates naturally, while a dependency that spans hundreds of events is never seen during a 50-event training and cannot be learned or tested under this protocol. The strong extrapolation of the HoTHP should be read together with this limitation.

These observations delimit where the HoTHP is expected to help. It is the right tool for processes whose relevance decays with time, fast or slow, which cover a large part of the event sequences of practical interest (self-exciting cascades, inhibition after an event, exponential-kernel Hawkes processes). It is not the right tool when the relevant structure is flat or periodic rather than decaying. A natural direction for future work is to combine the structural recency bias with a mechanism able to retain a few selected distant events, so that the model keeps its extrapolation stability without giving up long-range memory entirely.
